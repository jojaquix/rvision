...
fn main() {
    ...
    let  r1 = rect::TRect {
        a: point::TPoint { x: 0, y: 0 },
        b: point::TPoint { x: 20, y: 15 },
    };
    let tview = view::TView::new(r1, Some(Rc::downgrade(&tgroup)));
    tview.draw();
    tview.write_char(1, 1, 'x', 10, 2);
    

    let r2 = rect::TRect {
        a: point::TPoint { x: 21, y: 0 },
    ...
    tview2.draw();
    tview2.write_char(10, 1, 'y', 10, 2);
    screen::write_char(31, 2, 'c');
    tview2.write_line(1, 1, 8, 1, 'h');
    tview2.write_line(1, 2, 8, 8, 'a');

    screen::set_cursor_pos(0, 25);
}
